...
int main()
  {
    ...
    // Initialization of server sockaddr data structure, TCP
	...
	// Initialization of server sockaddr data structure, UDP
	// most likelt unecessray but couldnt figure out how to use same sockaddr_in
    ...
    while(choice != ALLDONE )
    {
		...
		if(choice == DEVOWEL )
	    {
			...
			// SEND UDP "D" for Devowel
			if(sendto(UDPsocket, "D", strlen("D"), 0, (struct sockaddr *)&server1, sizeof(server1)) == -1)
			{
			...
			if( recvfrom(UDPsocket, messagebackUDP, MAX_WORD_LENGTH, 0, (struct sockaddr *)&server1, &len) > 0)
			{
				...
				printf("Server sent %ld bytes of     vowels on UDP: '%s'", strlen(messagebackUDP), strtok(messagebackUDP,"\n"));
			}
			else
			{
				// an error condition if the server dies unexpectedly 
	  ...
	  else if(choice == ENVOWEL )
	  {
			...
			printf("LENGTH 2: %d!\n", len1);
			// see what the server sends back 
			if(((bytes = recv(sockfd, messageback,(len + len1), 0)) > 0))
			{
				// make sure the message is null-terminated in C 
				messageback[bytes] = '\0';
				printf("Server sent %ld bytes of message using TCP: '%s'\n", strlen(messageback), strtok(messageback, "\n"));
			}
			else
			{
				// an error condition if the server dies unexpectedly 
